...
export const discoveryDecisionJsonSchema = {
  ...
  anyOf: [
    ...
    { properties: { kind: { const: "wait" }, durationMs: { type: "integer", minimum: 50, maximum: 2000 }, rationale: { type: "string" } }, required: ["kind", "durationMs", "rationale"], additionalProperties: false },
  ...
  $defs: {
    locator: { type: "object", properties: { strategy: { enum: ["role", "label", "text", "css", "coordinates"] }, value: { type: "string" }, name: { type: "string" }, frame: { type: "array", items: { type: "string" } }, logicalTarget: { type: "string" }, fallback: { type: "array", items: { $ref: "#/$defs/locator" } }, rationale: { type: "string" } }, required: ["strategy", "value", "name", "frame", "logicalTarget", "fallback", "rationale"], additionalProperties: false },
    value: { type: "object", anyOf: [{ properties: { source: { const: "literal" }, value: { type: "string" } }, required: ["source", "value"], additionalProperties: false }, { properties: { source: { const: "input" }, key: { type: "string" } }, required: ["source", "key"], additionalProperties: false }] },
    assertion: { anyOf: [
