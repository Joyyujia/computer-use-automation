...
export type ModelContext = { goal: string; targetUrl: string; availableBusinessOutcomes: Array<{ code: string; message: string }>; availableOutputs: Array<{ key: string; type: string; description: string }>; inputs: Record<string, unknown>; observation: Observation; history: Array<{ decision: DiscoveryDecision; outcome: string }>; remainingSteps: number };
  ...
  async decide(context: ModelContext): Promise<{ decision: DiscoveryDecision; usage?: Record<string, number>; responseId?: string }> {
    const response = await fetch("https://api.openai.com/v1/responses", {
      ...
      body: JSON.stringify({
        ...
        instructions: "You operate the UI that is already open at targetUrl to accomplish the goal. Return exactly one safe next decision inside the required decision envelope, with only the fields defined for that decision kind. Do not navigate unless the current observation is on the wrong approved page. Use only exact inputKey names from inputs and exact outputKey names from availableOutputs. Prefer observed role/label targets for actions and copy an observed extractable target for extraction; never invent controls or selectors. Extract every required availableOutputs key before returning complete. Only report a business_outcome whose code appears in availableBusinessOutcomes and whose assertion is currently observable. Request a human when unsafe or stuck. Use wait when the application is visibly loading; never resubmit an action merely because its result is delayed. Every locator must include name (empty string if unused), frame (empty array if unused), logicalTarget (a stable semantic identifier), fallback, and rationale. A fallback is allowed only when it names the same logicalTarget as the primary locator. Visible and text assertions require a locator; URL assertions use the URL assertion branch without a locator.",
