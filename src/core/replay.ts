...
export async function replay(raw: unknown, inputs: Record<string, unknown>, options: {
  ...
  const applyRecoveries = async (): Promise<boolean> => {
    let applied = false;
    for (const recovery of capability.recoveries) {
      ...
      for (const recoveryStep of recovery.steps) {
        ...
        if (recoveryStep.action === "assert" && recoveryStep.assertion) { if (!await surface.matches(recoveryStep.assertion, inputs)) throw new Error(`Recovery assertion failed: ${recoveryStep.id}`); }
        else { await surface.execute(recoveryStep, resolveValue(recoveryStep, inputs)); await enforceBrowserState(page, options.policy ?? defaultPolicy); }
      }
  ...
  const handleIntervention = async (intervention: Capability["interventions"][number], step: Step): Promise<RunResult | "retry_step" | "verify_then_continue"> => {
    ...
    const observation = evidence.sanitize(await surface.observe());
  ...
  const waitForCompetingState = async (step: Step): Promise<RunResult | undefined> => {
    ...
    while (Date.now() <= deadline) {
      ...
      if (await applyRecoveries()) { await sleep(50); continue; }
  ...
  try {
    steps: for (current of capability.steps) {
      ...
      if (current.action === "extract") { const terminal = await waitForCompetingState(current); if (terminal) return terminal; }
      try { enforcePolicy(current, options.policy ?? defaultPolicy, options.confirmedStepIds?.includes(current.id)); }
        ...
        const result = await handleIntervention(policyIntervention, current); if (typeof result !== "string") return result; enforcePolicy(current, options.policy ?? defaultPolicy, true);
      }
      await evidence.event({ type: "step_started", stepId: current.id, action: current.action });
      const value = resolveValue(current, inputs); let extracted: string | undefined; let lastError: unknown;
      for (let attempt = 0; attempt <= current.retries; attempt++) {
        try {
          if (current.action === "assert" && current.assertion) { if (!await surface.matches(current.assertion, inputs)) throw new Error(`Assertion failed: ${current.id}`); }
          else { extracted = await surface.execute(current, value); await enforceBrowserState(page, options.policy ?? defaultPolicy); }
      ...
      if (lastError) throw lastError; if (current.outputKey) { outputs[current.outputKey] = extracted; if (capability.outputs[current.outputKey]?.sensitive) evidence.addSensitiveValues([extracted]); }
    ...
    const observation = evidence.sanitize(await surface.observe()); const evidencePath = evidence.path("failure-observation.json"); await writeFile(evidencePath, JSON.stringify(observation, null, 2) + "\n").catch(() => undefined);
    ...
    const result: RunResult = { status: "failure", runId, error: { category, stepId: current?.id, message, expected, observed, evidencePath, recoverable: false }, evidencePath: evidence.directory };
