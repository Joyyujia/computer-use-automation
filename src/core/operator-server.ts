...
export function createOperatorServer(registry: SessionRegistry, handoffs: HandoffCoordinator, port = 4174): Promise<Server> {
  ...
  app.post("/api/intervention/:id/click", async (req, res) => { try { const item = handoffs.get(req.params.id); const session = registry.get(sessionId(item)); if (session.owner !== "human") throw new Error("Human does not hold control"); await session.page.mouse.click(Number(req.body.x), Number(req.body.y)); handoffs.record(item.id, `Clicked (${req.body.x}, ${req.body.y})`); res.json({ ok: true }); } catch (e) { res.status(409).json({ error: String(e) }); } });
  app.post("/api/intervention/:id/type", async (req, res) => { try { const item = handoffs.get(req.params.id); const session = registry.get(sessionId(item)); if (session.owner !== "human") throw new Error("Human does not hold control"); await session.page.keyboard.type(String(req.body.text)); handoffs.record(item.id, "Typed redacted text"); res.json({ ok: true }); } catch (e) { res.status(409).json({ error: String(e) }); } });
  app.post("/api/intervention/:id/resume", (req, res) => { try { const current = handoffs.get(req.params.id); registry.transfer(sessionId(current), "human", "automation"); res.json(handoffs.resume(current.id)); } catch (e) { res.status(409).json({ error: String(e) }); } });
