...
export async function discover(options: {
  ...
  const decisions: DiscoveryDecision[] = []; const modelResponseIds: string[] = []; const history: Array<{ decision: DiscoveryDecision; outcome: string }> = []; const outputs: Record<string, unknown> = {}; const repeated = new Map<string, number>();
  ...
  try {
    ...
    for (let index = 1; index <= (options.maxSteps ?? 12); index++) {
      ...
      const modelCall = options.model.decide({ goal: safeGoal, inputs: Object.fromEntries(Object.keys(options.inputs).map(key => [key, "[SUPPLIED]"])), observation, history, remainingSteps: (options.maxSteps ?? 12) - index });
      const answer = await withTimeout(modelCall, options.modelTimeoutMs ?? 15_000);
      const decision = answer.decision; decisions.push(decision); if (answer.responseId) modelResponseIds.push(answer.responseId); await evidence.event({ type: "model_decision", index, decision, usage: answer.usage, responseId: answer.responseId });
    ...
    const artifact = compileArtifact({ goal: safeGoal, entrypoint: options.entrypoint, runId, model: options.model.name, decisions, inputKeys: Object.keys(options.inputs), inputDefinitions, checkpoint: proposedCheckpoint, profile, sensitiveValues, createdFromLiveRun: options.model.isLive === true, modelResponseIds });
    ...
    const failureObservation = evidence.path("failure-observation.json"); await writeFile(failureObservation, JSON.stringify(evidence.sanitize(await surface.observe()), null, 2) + "\n").catch(() => undefined);
