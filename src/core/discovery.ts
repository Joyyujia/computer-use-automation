...
import type { ModelAdapter, ModelContext } from "./model-adapter.js";
...
export async function discover(options: {
  ...
  const templatedGoal = Object.entries(options.inputs).reduce((goal, [key, value]) => typeof value === "string" && value ? goal.replaceAll(value, `[INPUT:${key}]`) : goal, options.goal);
  const safeGoal = evidence.sanitize(templatedGoal);
  ...
  try {
    ...
    try {
      await enforceBrowserState(page, options.policy ?? defaultPolicy, { allowInitialBlank: true });
    ...
    for (let index = 1; index <= (options.maxSteps ?? 12); index++) {
      ...
      const modelContext: ModelContext = { goal: safeGoal, targetUrl: redactUrl(options.entrypoint), availableBusinessOutcomes: profile.businessOutcomes.map(({ code, message }) => ({ code, message })), availableOutputs: Object.entries(profile.outputs).map(([key, output]) => ({ key, type: output.type, description: output.description })), inputs: Object.fromEntries(Object.keys(options.inputs).map(key => [key, "[SUPPLIED]"])), observation, history, remainingSteps: (options.maxSteps ?? 12) - index };
      const modelCall = options.model.decide(evidence.sanitize(modelContext));
      ...
      const invalidReference = decision.kind === "fill" && !inputDefinitions[decision.inputKey] ? `Unknown inputKey ${decision.inputKey}; choose one of ${Object.keys(inputDefinitions).join(", ")}` : decision.kind === "extract" && !profile.outputs[decision.outputKey] ? `Unknown outputKey ${decision.outputKey}; choose one of ${Object.keys(profile.outputs).join(", ")}` : undefined;
      if (invalidReference) { await evidence.event({ type: "model_decision_rejected", index, reason: invalidReference }); history.push({ decision, outcome: `Rejected without execution: ${invalidReference}` }); continue; }
      ...
      if (decision.kind === "complete") {
        const missingOutputs = Object.keys(profile.outputs).filter(key => !(key in outputs));
        if (missingOutputs.length > 0) { const reason = `Required outputs not yet extracted: ${missingOutputs.join(", ")}`; await evidence.event({ type: "model_decision_rejected", index, reason }); history.push({ decision, outcome: `Rejected without execution: ${reason}` }); continue; }
        proposedCheckpoint = decision.checkpoint; break;
      }
      ...
      try { await enforceBrowserState(page, options.policy ?? defaultPolicy); enforcePolicy(step, options.policy ?? defaultPolicy); }
      catch (error) {
